 ...
 * @version 1.0
 */
public class RoupasCorLookupController extends LookupController {

    private Connection conn = null;
    // public static VOListResponse veiculoImp;

    public RoupasCorLookupController(Connection conn) {
        this.conn = conn;
        this.setLookupDataLocator(new LookupDataLocator() {
            ...
            public Response validateCode(String code) {
                Statement stmt = null;
                try {
                    stmt = RoupasCorLookupController.this.conn.createStatement();
                    ResultSet rset = stmt.executeQuery(
                            "select cores.id_cor, cores.cor, cores.abrev_cor from cores where id_cor='"
                            + code + "'");
                    ArrayList list = new ArrayList();
                    while (rset.next()) {
                        CoresVO vo = new CoresVO();

                        vo.setIdCor(rset.getInt(1));
                        vo.setCor(rset.getString(2));
                        vo.setAbrevCor(rset.getString(3));
            ...
            public Response loadData(
                ...
                try {
                    stmt = RoupasCorLookupController.this.conn.createStatement();
                    ResultSet rset = stmt.executeQuery("select cores.id_cor, cores.cor, cores.abrev_cor from cores");
        ...
        this.setLookupValueObjectClassName("br.com.jumbo.vo.CoresVO");
         this.addLookup2ParentLink("idCor", "idCor");
        this.addLookup2ParentLink("cor", "cor");
