...
import br.com.jumbo.vo.LookupVeiculoVO;
 ...
 * @version 1.0
 */
public class ColaboradorLookupController extends LookupController {

    private Connection conn = null;
    // public static VOListResponse veiculoImp;

    public ColaboradorLookupController(Connection conn) {
        this.conn = conn;
        this.setLookupDataLocator(new LookupDataLocator() {
            ...
            public Response validateCode(String code) {
                Statement stmt = null;
                try {
                    stmt = ColaboradorLookupController.this.conn.createStatement();
                    ResultSet rset = stmt.executeQuery(
                            "select veiculo.vei_id, veiculo.vei_marca, veiculo.vei_mod, veiculo.vei_cor,"
                            + "veiculo.vei_placa from veiculo where vei_id='"
                            + code + "'");
                    ArrayList list = new ArrayList();
                    while (rset.next()) {
                        LookupVeiculoVO vo = new LookupVeiculoVO();

                        vo.setVeiId(rset.getInt(1));
                        vo.setVeiMarca(rset.getString(2));
                        vo.setVeiMod(rset.getString(3));
                        vo.setVeiCor(rset.getString(4));
                        vo.setVeiPlaca(rset.getString(5));
                        list.add(vo);
                    }
                    if (list.size() > 0) {
            ...
            public Response loadData(
                ...
                try {
                    ...
                    ResultSet rset = stmt.executeQuery("select veiculo.vei_id,"
                            + "veiculo.vei_marca, veiculo.vei_mod, veiculo.vei_cor,"
                            + "veiculo.vei_placa from veiculo");
        ...
        this.setLookupValueObjectClassName("br.com.jumbo.vo.LookupVeiculoVO");
        this.addLookup2ParentLink("veiId", "veiId");
        this.addLookup2ParentLink("veiPlaca", "veiPlaca");
